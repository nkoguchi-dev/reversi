...
class BoardTest {
    ...
    fun testCreate() {
        ...
        assertEquals(Disk(DiskType.Light), board.getDisk(SquarePosition(HorizontalPosition.FOUR, VerticalPosition.FOUR)))
        assertEquals(Disk(DiskType.Light), board.getDisk(SquarePosition(HorizontalPosition.FIVE, VerticalPosition.FIVE)))
        assertEquals(Disk(DiskType.Dark), diskMap[SquarePosition(HorizontalPosition.FOUR, VerticalPosition.FIVE)])
        assertEquals(Disk(DiskType.Dark), diskMap[SquarePosition(HorizontalPosition.FIVE, VerticalPosition.FOUR)])
    }
    ...
    inner class TestPutDisk {
        ...
        fun testPutDisk() {
            val board = Board.create()
            val newBoard = board.putDisk(SquarePosition(HorizontalPosition.SIX, VerticalPosition.SIX), Disk(DiskType.Light))
            assertEquals(Disk(DiskType.Light), newBoard.getDisk(SquarePosition(HorizontalPosition.SIX, VerticalPosition.SIX)))
        }
        ...
        fun testPutDiskToAlreadyOccupiedPosition() {
            ...
            val exception = assertThrows(CustomIllegalArgumentException::class.java) {
                board.putDisk(SquarePosition(HorizontalPosition.FOUR, VerticalPosition.FOUR), Disk(DiskType.Light))
            }
