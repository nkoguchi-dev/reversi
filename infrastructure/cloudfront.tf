...
resource "aws_cloudfront_origin_access_control" "reversi_oac" {
  name                              = "reversi-web-front.s3.ap-northeast-1.amazonaws.com"
  description                       = "Origin Access Control for S3 bucket reversi-ui"
...
resource "aws_cloudfront_distribution" "reversi" {
  ...
  default_cache_behavior {
    ...
    target_origin_id       = aws_s3_bucket.reversi-ui.bucket_regional_domain_name
  }
  ...
  origin {
    domain_name              = aws_s3_bucket.reversi-ui.bucket_regional_domain_name
    origin_id                = aws_s3_bucket.reversi-ui.bucket_regional_domain_name
    origin_access_control_id = aws_cloudfront_origin_access_control.reversi_oac.id
  }
